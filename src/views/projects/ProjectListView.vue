...
async function load() {
  loading.value = true
  try {
    const { data } = await api.get('/admin/projects', { params: { per_page: 100 } })
...
<template>
  <div>
    <div class="mb-4 flex items-center justify-between">
      <h1 class="text-xl font-bold text-navy-900">{{ t('nav.projects') }}</h1>
      <RouterLink
        :to="{ name: 'projects-create', params: localeParams() }"
    ...
    <DataTable :columns="columns.map(c => ({...c, label: c.label.startsWith('common.') ? t(c.label) : c.label}))" :rows="displayRows" :loading="loading">
      <template #row-actions="{ row }">
        <RouterLink
          :to="{ name: 'projects-edit', params: localeParams({ id: row.id }) }"
